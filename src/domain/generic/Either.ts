...
export class Either<Error, Data> {
  ...
  flatMap<Data1>(
    fn: (data: Data) => Either<Error, Data1>,
  ): Either<Error, Data1> {
    return this.match({
      success: (data) => fn(data),
      error: (error) => new Either<Error, Data1>({ type: 'error', error }),
    });
  }

  flatMapError<Error1>(
    fn: (error: Error) => Either<Error1, Data>,
  ): Either<Error1, Data> {
    return this.match({
      success: (data) => new Either<Error1, Data>({ type: 'success', data }),
...
type EitherValue<Error, Data> =
  | EitherValueError<Error>
  | EitherValueSuccess<Data>;
